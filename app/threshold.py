...
class Threshold:
    ...
    def get_path_to_image(self, row: pd.DataFrame, path_to_folder: str) -> np.ndarray:
        ...
        path_to_image = os.path.join(path_to_folder, tp, f"{row['attachment_id']}.npy")
    ...
    def process_user_data(self, path_to_folder: str):
        ...
        for user_id, group in grouped:
            vectors = np.array([self.get_embedding(self.read_image(self.get_path_to_image(row, path_to_folder))) for _, row in group])
            print(vectors)
            if len(vectors) < 2:
                avg_distance = -100
            else:
